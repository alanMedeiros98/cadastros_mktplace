...
public class CategoriaService {
	...
	public void removerPor(int id) {
		if(id > 0) {
			...
			if (qtdeDeRestaurantes > 0) {
				throw new IllegalArgumentException("Não foi possivel excluir a categoria, Motivo: Existe(m) " + qtdeDeRestaurantes + " restaurantes vinculados a categoria.");
			}
	...
	private void validar(Categoria categoria) {
		if(categoria != null) {
			...
			if(isNomeinvalido) {
				throw new IllegalArgumentException("O nome da categoria é obrigatório e deve possuir entre 3 e 100 caracteres");
